...
NetworkAccessManager *DownloadItem::s_networkAccessManager = 0;
...
void DownloadItem::getFileName()
{
    ...
    fileName = QFileDialog::getSaveFileName(this, tr("Save File"), "",  tr("Airfoil data (*.dat)"));
    if (fileName.isEmpty()) {
        m_reply->close();
        return;
    }
...
DownloadManager::DownloadManager(QWidget *parent)
    ...
    , m_manager(DownloadItem::networkAccessManager())
    , m_iconProvider(0)
{
    setupUi(this);
...
NetworkAccessManager *DownloadItem::networkAccessManager() {
